/* Smalltalk from Squeak4.5 with VMMaker 4.13.6 translated as C source on 4 October 2014 3:42:05 pm */
/* Automatically generated by
	JSSmartSyntaxPluginCodeGenerator VMMakerJS-bf.2 uuid: 354e3141-358c-44ff-b3cf-a47b08b40e20
   from
...
function anyBitOfBytesfromto(aBytesOop, start, stopArg) {
	...
	for (ix = (firstByteIx + 1), ixLimiT = (lastByteIx - 1); ix <= ixLimiT; ix++) {
...
function cDigitSublenwithleninto(pByteSmall, smallLen, pByteLarge, largeLen, pByteRes) {
	...
	z = 0;
	for (i = 0, iLimiT = (smallLen - 1); i <= iLimiT; i++) {
		z = (z + (pByteLarge[i])) - (pByteSmall[i]);
		pByteRes[i] = (z & 255);
		z = z >> 8;
	}
	for (i = smallLen, iLimiT = (largeLen - 1); i <= iLimiT; i++) {
		z += pByteLarge[i];
		pByteRes[i] = (z & 255);
		z = z >> 8;
	}
...
function primGetModuleName() {
	...
	strPtr = strOop.bytes;
	for (i = 0, iLimiT = (strLen - 1); i <= iLimiT; i++) {
