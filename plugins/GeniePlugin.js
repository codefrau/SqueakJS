/* Smalltalk from Squeak4.5 with VMMaker 4.13.6 translated as JS source on 13 November 2014 7:11 pm */
/* Automatically generated by
	JSSmartSyntaxPluginCodeGenerator VMMakerJS-bf.16 uuid: aa9fd3da-7901-4c62-ae34-c166f1492d3b
   from
...
var moduleName = "GeniePlugin v2.0 13 November 2014 (e)";
...
function primSameClassAbsoluteStrokeDistanceMyPoints_otherPoints_myVectors_otherVectors_mySquaredLengths_otherSquaredLengths_myAngles_otherAngles_maxSizeAndReferenceFlag_rowBase_rowInsertRemove_rowInsertRemoveCount() {
	...
	for (j = 1; j <= jLimiT; j++) {
		jM1 = j - 1;
		insertRemove = (insertRemove + ((otherSquaredLengths[jM1] + cSquaredDistanceFromto(new Int32Array(otherPoints.buffer, otherPoints.byteOffset + 4 * ((jM1 << 1))), myPoints)) >>> 7)) + additionalMultiInsertRemoveCost;
	...
	for (i = 1; i <= myVectorsSize; i++) {
		...
		iM1T2 = iM1 << 1;
		substBase = rowBase[0];
		insertRemove = (insertRemove + ((mySquaredLengths[iM1] + cSquaredDistanceFromto(new Int32Array(myPoints.buffer, myPoints.byteOffset + 4 * (iM1T2)), otherPoints)) >>> 7)) + additionalMultiInsertRemoveCost;
		...
		for (j = 1; j <= jLimiT; j++) {
			...
			remove = (mySquaredLengths[iM1] + cSquaredDistanceFromto(new Int32Array(myPoints.buffer, myPoints.byteOffset + 4 * (iM1T2)), new Int32Array(otherPoints.buffer, otherPoints.byteOffset + 4 * ((j << 1))))) >>> 7;
			...
			insert = (otherSquaredLengths[jM1] + cSquaredDistanceFromto(new Int32Array(otherPoints.buffer, otherPoints.byteOffset + 4 * (jM1T2)), new Int32Array(myPoints.buffer, myPoints.byteOffset + 4 * ((i << 1))))) >>> 7;
			...
			if (forReference) {
				...
				subst = ((cSquaredDistanceFromto(new Int32Array(otherVectors.buffer, otherVectors.byteOffset + 4 * (jM1T2)), new Int32Array(myVectors.buffer, myVectors.byteOffset + 4 * (iM1T2))) + cSquaredDistanceFromto(new Int32Array(otherPoints.buffer, otherPoints.byteOffset + 4 * (jM1T2)), new Int32Array(myPoints.buffer, myPoints.byteOffset + 4 * (iM1T2)))) * (16 + cSubstAngleFactorFromto(otherAngles[jM1], myAngles[iM1]))) >>> 11;
				substBase += subst;
			}
