/* Smalltalk from Squeak4.5 with VMMaker 4.13.6 translated as C source on 3 October 2014 4:08:10 am */
/* Automatically generated by
	JSSmartSyntaxPluginCodeGenerator * VMMakerJS-bf.7 uuid: 93ada3e0-e3be-491a-8c7d-64a09acb0759
   from
...
function anyBitOfBytesfromto(aBytesOop, start, stopArg) {
	...
	if (firstByteIx === lastByteIx) {
		mask = ((rightShift > 31 ? 0 : 255 << rightShift)) & ((leftShift > 31 ? 0 : 255 >>> leftShift));
		digit = digitOfBytesat(magnitude, firstByteIx);
		return (digit & mask) !== 0;
	}
	if (((rightShift > 31 ? 0 : (digitOfBytesat(magnitude, firstByteIx)) >>> rightShift)) !== 0) {
		return true;
	}
	for (ix = (firstByteIx + 1), ixLimiT = (lastByteIx - 1); ix <= ixLimiT; ix += 1) {
		if ((digitOfBytesat(magnitude, ix)) !== 0) {
			return true;
		}
	}
	if ((((leftShift > 31 ? 0 : (digitOfBytesat(magnitude, lastByteIx)) << leftShift)) & 255) !== 0) {
		return true;
	}
	return false;
}
...
function byteSizeOfBytes(bytesOop) {
	return (bytesOop.bytes ? bytesOop.bytes.length : bytesOop.words ? bytesOop.words.length : interpreterProxy.slotSizeOf(bytesOop));
}
...
function bytesgrowTo(aBytesObject, newLen) {
	...
	newBytes = interpreterProxy.instantiateClassindexableSize((aBytesObject.sqClass ? aBytesObject.sqClass : interpreterProxy.classSmallInteger()), newLen);
;
	oldLen = aBytesObject.bytes ? aBytesObject.bytes.length : 0;
	if (oldLen < newLen) {
		copyLen = oldLen;
	} else {
		copyLen = newLen;
...
function cDigitLshiftfromlentolen(shiftCount, pFrom, lenFrom, pTo, lenTo) {
	...
	for (i = 0; i <= limit; i += 1) {
		digit = pFrom[i];
		pTo[i + digitShift] = ((carry | ((bitShift > 31 ? 0 : digit << bitShift))) & 255);
		carry = (rshift > 31 ? 0 : digit >>> rshift);
	}
	if (!(carry === 0)) {
		pTo[lenTo - 1] = carry;
	}
...
function cDigitOfCSIat(csi, ix) {
	...
	if (csi < 0) {
		;
		return ((((ix - 1) * 8) > 31 ? 0 : (0 - csi) >>> ((ix - 1) * 8))) & 255;
	} else {
		return ((((ix - 1) * 8) > 31 ? 0 : csi >>> ((ix - 1) * 8))) & 255;
	}
}
...
function cDigitRshiftfromlentolen(shiftCount, pFrom, fromLen, pTo, toLen) {
	...
	carry = (bitShift > 31 ? 0 : (pFrom[digitShift]) >>> bitShift);
	start = digitShift + 1;
	limit = fromLen - 1;
	for (j = start; j <= limit; j += 1) {
		digit = pFrom[j];
		pTo[j - start] = ((carry | ((leftShift > 31 ? 0 : digit << leftShift))) & 255);
		carry = (bitShift > 31 ? 0 : digit >>> bitShift);
	}
	if (!(carry === 0)) {
		pTo[toLen - 1] = carry;
	}
...
function cDigitSublenwithleninto(pByteSmall, smallLen, pByteLarge, largeLen, pByteRes) {
	...
	z = 0;
	for (i = 0, iLimiT = (smallLen - 1); i <= iLimiT; i += 1) {
		z = (z + (pByteLarge[i])) - (pByteSmall[i]);
		pByteRes[i] = (z & 255);
		z = (z >> 8);
	}
	for (i = smallLen, iLimiT = (largeLen - 1); i <= iLimiT; i += 1) {
		z += pByteLarge[i];
		pByteRes[i] = (z & 255);
		z = (z >> 8);
	}
...
function digitLshift(aBytesOop, shiftCount) {
	...
	oldLen = aBytesOop.bytes ? aBytesOop.bytes.length : 0;
	if (((highBit = cDigitHighBitlen(aBytesOop.bytes, oldLen))) === 0) {
		return 0;
	}
	newLen = (((highBit + shiftCount) + 7) >> 3);
	newBytes = interpreterProxy.instantiateClassindexableSize((aBytesOop.sqClass ? aBytesOop.sqClass : interpreterProxy.classSmallInteger()), newLen);
;
...
function digitRshiftlookfirst(aBytesOop, shiftCount, a) {
	...
	if (newBitLen <= 0) {
		...
		return interpreterProxy.instantiateClassindexableSize((aBytesOop.sqClass ? aBytesOop.sqClass : interpreterProxy.classSmallInteger()), 0);
	}
	newByteLen = ((newBitLen + 7) >> 3);
	newOop = interpreterProxy.instantiateClassindexableSize((aBytesOop.sqClass ? aBytesOop.sqClass : interpreterProxy.classSmallInteger()), newByteLen);
;
...
function digitAddLargewith(firstInteger, secondInteger) {
	...
	firstLen = firstInteger.bytes ? firstInteger.bytes.length : 0;
	secondLen = secondInteger.bytes ? secondInteger.bytes.length : 0;
	resClass = (firstInteger.sqClass ? firstInteger.sqClass : interpreterProxy.classSmallInteger());
...
;
		...
		if (((firstInteger.sqClass ? firstInteger.sqClass : interpreterProxy.classSmallInteger())) === (interpreterProxy.classLargeNegativeInteger())) {
			return interpreterProxy.primitiveFail();
		}
...
;
		...
		if (((secondInteger.sqClass ? secondInteger.sqClass : interpreterProxy.classSmallInteger())) === (interpreterProxy.classLargeNegativeInteger())) {
			return interpreterProxy.primitiveFail();
		}
		secondLarge = secondInteger;
	}
	firstLen = firstLarge.bytes ? firstLarge.bytes.length : 0;
	secondLen = secondLarge.bytes ? secondLarge.bytes.length : 0;
	if (firstLen < secondLen) {
		shortLen = firstLen;
...
function digitLength(oop) {
	if (typeof oop === "number") {
		...
		return oop.bytes ? oop.bytes.length : 0;
	}
}
...
function digitMontgomerytimesmodulomInvModB(firstLarge, secondLarge, thirdLarge, mInv) {
	...
	firstLen = firstLarge.bytes ? firstLarge.bytes.length : 0;
	secondLen = secondLarge.bytes ? secondLarge.bytes.length : 0;
	thirdLen = thirdLarge.bytes ? thirdLarge.bytes.length : 0;
...
function digitOfBytesat(aBytesOop, ix) {
	if (ix > (aBytesOop.bytes ? aBytesOop.bytes.length : 0)) {
		return 0;
	} else {
...
function digitSubLargewith(firstInteger, secondInteger) {
	...
	firstNeg = ((firstInteger.sqClass ? firstInteger.sqClass : interpreterProxy.classSmallInteger())) === (interpreterProxy.classLargeNegativeInteger());
	firstLen = firstInteger.bytes ? firstInteger.bytes.length : 0;
	secondLen = secondInteger.bytes ? secondInteger.bytes.length : 0;
	if (firstLen === secondLen) {
...
function highBitOfBytes(aBytesOop) {
	return cDigitHighBitlen(aBytesOop.bytes, aBytesOop.bytes ? aBytesOop.bytes.length : 0);
}
...
function isNormalized(anInteger) {
	...
	if (((anInteger.sqClass ? anInteger.sqClass : interpreterProxy.classSmallInteger())) === (interpreterProxy.classLargePositiveInteger())) {
...
function normalize(aLargeInteger) {
	// missing DebugCode;
	if (((aLargeInteger.sqClass ? aLargeInteger.sqClass : interpreterProxy.classSmallInteger())) === (interpreterProxy.classLargePositiveInteger())) {
...
function _primDigitBitShift() {
	...
	if (shiftCount >= 0) {
		...
		rShift = 0 - shiftCount;
		_return_value = normalize(digitRshiftlookfirst(aLarge, rShift, aLarge.bytes ? aLarge.bytes.length : 0));
		if (interpreterProxy.failed()) {
			return null;
		}
		interpreterProxy.popthenPush(3, _return_value);
...
function primGetModuleName() {
	...
	strPtr = strOop.bytes;
	for (i = 0, iLimiT = (strLen - 1); i <= iLimiT; i += 1) {
