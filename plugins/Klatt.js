...
/* Automatically generated by
   ...
   from
	KlattSynthesizerPlugin VMMaker-bf.351 uuid: af5fd014-b9f4-4d17-80fa-2abd7b2b263b
 */
...
var moduleName = "Klatt 7 October 2014 (e)";
var nlast = 0;
var nmod = 0;
var nopen = 0;
var nper = 0;
...
function addJitter() {
	pitch += (DIV((((nextRandom()) - 32767) * (frame[Jitter])), 32768.0)) * (frame[F0]);
}
...
function addShimmer() {
	...
	x1 += (DIV((((nextRandom()) - 32767) * (frame[Shimmer])), 32768.0)) * x1;
	if (x1 > 0) {
		x1 = 0;
	}
}
...
function antiResonatorfrequencybandwidth(index, freq, bw) {
	...
	arg = (DIV((0.0 - PI), samplingRate)) * bw;
	r = exp(arg);
	c = 0.0 - (r * r);
	arg = (DIV((PI * 2.0), samplingRate)) * freq;
...
function linearFromdB(aNumber) {
	return (Math.pow(2.0,(DIV((aNumber - 87.0), 6.0)))) * 32.767;
}
...
function rorark(roNumber, raNumber, rkNumber) {
	...
	if (ra <= 0.0) {
		...
		r = DIV((1.0 - ro), ra);
		d = 1.0 - (DIV(r, ((exp(r)) - 1.0)));
	}
	phi = PI * (rk + 1.0);
	cosphi = Math.cos(phi);
	sinphi = Math.sin(phi);
	rphid = ((DIV(ra, ro)) * phi) * d;
	...
	gamma = exp(DIV(-1.0, (ra * t0)));
	gammapwr = Math.pow(gamma,(t0 - te));
	b1 = gamma;
	c1 = DIV(((1.0 - gamma) * gammapwr), (1.0 - gammapwr));
...
function synthesizeFrameintostartingAt(aKlattFrame, buffer, startIndex) {
	...
	while (index <= top) {
		...
		noise = DIV((((nextRandom()) - 32768)), 4.0);
		noise += 0.75 * nlast;
		...
		vlast = voice;
		if (nper < nopen) {
			voice += DIV((turbulence * (((nextRandom()) - 32768))), 4.0);
		}
