/* Smalltalk from Squeak4.5 with VMMaker 4.13.6 translated as JS source on 7 October 2014 4:13:20 pm */
...
var moduleName = "KedamaPlugin2 7 October 2014 (e)";
var randA = 0;
var randM = 0;
var randQ = 0;
var randR = 0;
...
function degreesFromXy(x, y) {
	...
	if (x === 0.0) {
		...
		if (x >= 0.0) {
			if (y >= 0.0) {
				return DIV(theta, 0.0174532925199433);
			} else {
				return 360.0 + (DIV(theta, 0.0174532925199433));
			}
		} else {
			return 180.0 + (DIV(theta, 0.0174532925199433));
		}
	}
	return 0.0;
}
...
function getHeadingArrayInto() {
	...
	for (i = 0; i <= (size - 1); i++) {
		...
		heading = DIV(heading, 0.0174532925199433);
...
function makeMaskLog() {
	...
	for (i = 0; i <= (dataSize - 1); i++) {
		...
		if (data === 0) {
			...
			alpha = (((DIV(255.0, maxLog)) * (Math.log(data)))|0);
		}
		if (alpha > 255) {
			alpha = 255;
		}
...
function primUpHill() {
	...
	if (-1 === maxValX) {
		...
		ret = (degreesFromXy((maxValX - turtleX), (maxValY - turtleY))) + 90.0;
	}
	if (interpreterProxy.failed()) {
		return null;
	}
...
function primitiveDivArrays() {
	...
	if (isRcvrWords) {
		if (isArgWords) {
			...
			wordsResult = resultOop.words;
			for (i = 0; i <= (length - 1); i++) {
				wordsResult[i] = (DIV((wordsRcvr[i]), (wordsArg[i])));
			}
		} else {
			wordsRcvr = rcvrOop.words;
			floatsArg = argOop.wordsAsFloat32Array();
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((wordsRcvr[i]), (floatsArg[i])));
			}
		}
	} else {
		if (isArgWords) {
			...
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((floatsRcvr[i]), (wordsArg[i])));
			}
		} else {
			floatsRcvr = rcvrOop.wordsAsFloat32Array();
			floatsArg = argOop.wordsAsFloat32Array();
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((floatsRcvr[i]), (floatsArg[i])));
			}
		}
	}
	interpreterProxy.pop(4);
...
function primitiveDivScalar() {
	...
	if (isRcvrWords) {
		if (isArgInt) {
			...
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((wordsRcvr[i]), floatArg));
			}
		}
	} else {
		if (isArgInt) {
			...
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((floatsRcvr[i]), intArg));
			}
		} else {
			floatsRcvr = rcvrOop.wordsAsFloat32Array();
			floatArg = interpreterProxy.floatValueOf(argOop);
			floatsResult = resultOop.wordsAsFloat32Array();
			for (i = 0; i <= (length - 1); i++) {
				floatsResult[i] = (DIV((floatsRcvr[i]), floatArg));
			}
		}
	}
	interpreterProxy.pop(4);
	...
	if (isRcvrWords) {
		if (isArgWords) {
			...
			for (i = 0; i <= (length - 1); i++) {
				...
				floatResult = DIV(wordRcvr, floatArg);
		...
		if (isArgWords) {
			...
			for (i = 0; i <= (length - 1); i++) {
				...
				wordArg = wordsArg[i];
				floatResult = DIV(floatRcvr, wordArg);
				floatResult = Math.floor(floatResult);
			...
			for (i = 0; i <= (length - 1); i++) {
				floatRcvr = floatsRcvr[i];
				floatArg = floatsArg[i];
				floatResult = DIV(floatRcvr, floatArg);
				floatResult = Math.floor(floatResult);
...
function primitiveRemScalar() {
	...
	if (isRcvrWords) {
		...
		if (isArgInt) {
			...
			for (i = 0; i <= (length - 1); i++) {
				floatRcvr = floatsRcvr[i];
				floatResult = DIV(floatRcvr, intArg);
				floatResult = Math.floor(floatResult);
...
function radiansToDegrees(radians) {
	...
	/* inline: true */;
	degrees = DIV(radians, 0.0174532925199433);
	deg = 90.0 - degrees;
	if (!(deg > 0.0)) {
		deg += 360.0;
	}
