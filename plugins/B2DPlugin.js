/* Smalltalk from Squeak4.5 with VMMaker 4.13.6 translated as JS source on 13 November 2014 7:10:59 pm */
...
var loadBBFn = null;
var moduleName = "B2DPlugin 13 November 2014 (e)";
var objBuffer = null;
var objUsed = 0;
var spanBuffer = null;
var workBuffer = null;
...
function allocateGETEntry(nSlots) {
	...
	aetBuffer = new Int32Array(aetBuffer.buffer, aetBuffer.byteOffset + 4 * (nSlots));
	return true;
}
...
function allocateObjEntry(nSlots) {
	...
	getBuffer = new Int32Array(getBuffer.buffer, getBuffer.byteOffset + 4 * (nSlots));
	return true;
}
...
function bezierUpdateDataOf(bezier) {
	return new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (bezier + GBUpdateData));
}
...
function colorTransform() {
	return new Float32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWColorTransform));
}

function colormapOf(bmFill) {
	return new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (bmFill + GBColormapOffset));
}
...
function edgeTransform() {
	return new Float32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWEdgeTransform));
}
...
function gradientRampOf(fill) {
	return new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (fill + GFRampOffset));
}
...
function initializeGETProcessing() {
	...
	getUsedPut(0);
	aetUsedPut(0);
	getBuffer = new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (objUsed));

	/* Create the global edge table */

	aetBuffer = new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (objUsed));
...
function loadWorkBufferFrom(wbOop) {
	...
	objBuffer = new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (objStartGet()));
	getBuffer = new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (objUsedGet()));

	/* Make sure we don't exceed the work buffer */

	aetBuffer = new Int32Array(getBuffer.buffer, getBuffer.byteOffset + 4 * (getUsedGet()));
...
function point1Get() {
	return new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWPoint1));
}

function point2Get() {
	return new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWPoint2));
}

function point3Get() {
	return new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWPoint3));
}

function point4Get() {
	return new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWPoint4));
}
...
function primitiveCopyBuffer() {
	...
	src = new Int32Array(src.buffer, src.byteOffset + 4 * (wbTopGet()));
	dst = new Int32Array(dst.buffer, dst.byteOffset + 4 * (wbTopGet() + diff));
...
function primitiveInitializeBuffer() {
	...
	workBufferPut(wbOop);
	objBuffer = new Int32Array(workBuffer.buffer, workBuffer.byteOffset + 4 * (GWHeaderSize));
...
function wideBezierUpdateDataOf(bezier) {
	return new Int32Array(objBuffer.buffer, objBuffer.byteOffset + 4 * (bezier + GBWideUpdateData));
}
